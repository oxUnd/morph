...
{
	reply_json(r, 401, "{\"error\":\"unauthorized\"}");
}

void reply_403(request_t *r)
{
	reply_json(r, 403, "{\"error\":\"forbidden\"}");
}

void reply_404(request_t *r)
{
	reply_json(r, 404, "{\"error\":\"not_found\"}");
}
