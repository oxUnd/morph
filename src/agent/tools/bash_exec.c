...
int bash_exec_init(struct tool_registry *reg, struct tool_context *tctx)
{
	...
	int rc = tool_register(reg, &(struct tool_spec){ .origin = TOOL_ORIGIN_BUILTIN, .name = "bash_exec", .description = "Execute a shell command in a restricted sandboxed subprocess. "
		...
		"Use sandbox_permissions=use_default unless additional directories "
		"are needed; then use with_additional_permissions with only the "
		"smallest required absolute directories. Deletion and rename require "
		"delete permission; write permission alone is insufficient. "
		"If error.code=sandbox_denied, retry the same command with the "
		"narrow additional permissions indicated by the failure; do not "
		"claim success from its exit code. Use require_escalated only when "
		"narrow directory permissions cannot work; it always requires "
		"approval and is unavailable in server mode. "
