		       ...
		       const char *session_id)
{
	...
	rc = tool_register(reg, "tool_create",
			   "Create or update a dynamic JavaScript tool for this agent session. The source_js runs in embedded QuickJS, not Node.js. It must define global function run(args) or async function run(args), and return a JSON-serializable value. Do not use Node-only APIs or syntax: no require(), no node: imports, no fs/http/https/child_process modules, no process, Buffer, __dirname, or npm packages. Use plain JavaScript plus the morph host APIs only. Available host APIs: morph.fs.readText(path), morph.fs.writeText(path, text), morph.env.get(name), morph.exec(command), morph.fetch(url). morph.fetch(url) returns a Response-like object: const response = await morph.fetch(url); const html = await response.text(); response.status and response.ok are available. Request capabilities when using host APIs: fs_read, fs_write, env, shell/process, network. If a dynamic tool with the same name already exists, tool_create updates it; non-dynamic tools cannot be overwritten.",
