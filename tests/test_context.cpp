...
TEST_F(ContextTest, CreateMessage) {
	struct message_list *m = msg_list_create("user", "hello world", 3);
	ASSERT_NE(m, nullptr);
...
TEST_F(ContextTest, CreateNullRole) {
	struct message_list *m = msg_list_create(nullptr, "content", 1);
	ASSERT_NE(m, nullptr);
	EXPECT_STREQ(m->role, "");
	msg_list_destroy(m);
}
...
TEST_F(ContextTest, AppendMessages) {
	...
	msg_list_append(&head, msg_list_create("assistant", "hello", 1));
	msg_list_append(&head, msg_list_create("user", "how are you", 3));
...
TEST_F(ContextTest, TokenCountWithTokenizer) {
	struct message_list *head = nullptr;
	msg_list_append(&head, msg_list_create("user", "hello world", 0));
	msg_list_append(&head, msg_list_create("assistant", "hi there", 0));
...
TEST_F(ContextTest, TokenCountWithoutTokenizer) {
	struct message_list *head = nullptr;
	msg_list_append(&head, msg_list_create("user", "hello", 5));
	msg_list_append(&head, msg_list_create("assistant", "hi", 2));
...
TEST_F(ContextTest, NeedsCompress) {
	...
	struct message_list *head = nullptr;
	msg_list_append(&head, msg_list_create("user", "short message", 0));
	EXPECT_EQ(context_needs_compress(head, tok, &cfg), 0);

	/* Create text long enough to exceed 80 tokens (threshold = 100 * 0.8 = 80).
	 * Chinese characters count as ~2 tokens each, so ~50 chars should exceed it. */
	char long_text[600];
	int pos = 0;
	for (int i = 0; i < 50 && pos < 598; i++) {
		long_text[pos++] = (char)0xE4; /* 中 = U+4E2D */
		long_text[pos++] = (char)0xB8;
		long_text[pos++] = (char)0xAD;
	}
	long_text[pos] = '\0';
	msg_list_append(&head, msg_list_create("user", long_text, 0));
	int count = context_token_count(head, tok);
...
TEST_F(ContextTest, NeedsCompressNullCfg) {
	struct message_list *head = nullptr;
	msg_list_append(&head, msg_list_create("user", "test", 1));
